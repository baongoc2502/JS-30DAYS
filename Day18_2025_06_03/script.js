...
// Thêm trình lắng nghe sự kiện cho nút tìm kiếm
document.querySelector('.product-input button:last-of-type').addEventListener('click', searchProduct);
...
function searchProduct() {
    ...
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const idCell = row.getElementsByTagName('td')[0]; // ID là ô đầu tiên
        if (idCell) {
            const productId = idCell.textContent.toLowerCase();
            if (productId.includes(searchTerm) || searchTerm === '') {
                row.style.display = ''; // Hiển thị hàng nếu khớp hoặc ô tìm kiếm trống
            } else {
